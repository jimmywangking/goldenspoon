...
public interface UserPageContentMapper extends BaseMapper<UserPageContent> {
    ...
    @Select("SELECT * FROM user_page_content WHERE user_id = #{userId} AND page_code = #{pageCode} AND is_deleted = false ORDER BY version DESC LIMIT 1")
    UserPageContent findByUserAndPage(@Param("userId") Long userId, @Param("pageCode") String pageCode);

    @Select("SELECT * FROM user_page_content WHERE user_id = #{userId} AND page_code = #{pageCode} AND is_deleted = false ORDER BY version DESC")
    List<UserPageContent> listVersions(@Param("userId") Long userId, @Param("pageCode") String pageCode);
    ...
    List<UserPageContent> listByPage(@Param("pageCode") String pageCode);
    ...
    List<UserPageContent> listByPageAndOrg(@Param("pageCode") String pageCode, @Param("orgId") Long orgId);

    @Select("SELECT COUNT(*) FROM user_page_content WHERE user_id = #{userId} AND page_code = #{pageCode} AND is_deleted = false")
    int countByUserAndPage(@Param("userId") Long userId, @Param("pageCode") String pageCode);
}
