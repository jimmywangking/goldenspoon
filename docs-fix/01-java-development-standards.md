...
```mermaid
flowchart TB
    CONTROLLER["Controller<br>@Valid 参数校验"]
    VALIDATE_FAIL["参数校验失败<br>抛出 MethodArgumentNotValidException"]
    SERVICE["Service<br>@Transactional 事务边界"]
    DB_EXCEPTION["DB 异常<br>DataAccessException"]
    BUSINESS_EXCEPTION["业务异常<br>BusinessException"]
    GLOBAL_HANDLER["GlobalExceptionHandler<br>@RestControllerAdvice"]
    RESULT["统一响应<br>Result{code, message, data}"]

    CONTROLLER -->|"校验通过"| SERVICE
    CONTROLLER -->|"校验失败"| VALIDATE_FAIL
    SERVICE -->|"成功"| RESULT
    SERVICE -->|"业务异常"| BUSINESS_EXCEPTION
    SERVICE -->|"DB异常"| DB_EXCEPTION
    VALIDATE_FAIL --> GLOBAL_HANDLER
    BUSINESS_EXCEPTION --> GLOBAL_HANDLER
    DB_EXCEPTION --> GLOBAL_HANDLER
    GLOBAL_HANDLER --> RESULT

    style CONTROLLER fill:#fff3e0,stroke:#e65100
    style SERVICE fill:#e3f2fd,stroke:#1565c0
    style GLOBAL_HANDLER fill:#f3e5f5,stroke:#4a148c
...
flowchart LR
    ...
    subgraph S["Spring Boot 后端"]
        direction TB
        CTRL["Controller<br>参数校验 / 协议适配"]
        SVC["Service<br>业务逻辑 / 事务管理"]
        MAPPER["Mapper<br>数据访问 / CRUD"]
    end
    subgraph DB["数据库"]
        PG[("PostgreSQL")]
        REDIS[("Redis 缓存")]
    ...
    style MAPPER fill:#e8f5e9,stroke:#2e7d32
