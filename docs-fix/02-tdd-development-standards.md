...
```mermaid
flowchart LR
    RED["RED<br>写失败测试"] -->|"mvn test 失败"| GREEN["GREEN<br>写最少代码"]
    GREEN -->|"mvn test 通过"| REFACTOR["REFACTOR<br>重构优化"]
    REFACTOR -->|"mvn test 仍通过"| RED
    REFACTOR -->|"回归失败"| GREEN

    style RED fill:#ffcdd2,stroke:#c62828
    style GREEN fill:#c8e6c9,stroke:#2e7d32
    style REFACTOR fill:#fff9c4,stroke:#f9a825
```
...
```mermaid
graph TB
    E2E["E2E 测试<br>少量<br>分钟级"]
    INT["集成测试<br>中等数量<br>秒级"]
    UNIT["单元测试<br>大量<br>毫秒级"]

    style UNIT fill:#e8f5e9,stroke:#2e7d32,stroke-width:3px
...
```mermaid
flowchart LR
    subgraph STEP1["STEP 1: RED"]
        direction TB
        WRITE_TEST["写测试用例"]
        RUN_TEST["运行测试<br>确认失败"]
    end
    subgraph STEP2["STEP 2: GREEN"]
        direction TB
        WRITE_IMPL["写最少实现代码"]
        VERIFY["运行测试<br>确认通过"]
    end
    subgraph STEP3["STEP 3: REFACTOR"]
        OPTIMIZE["重构代码结构"]
        REGRESS["重新跑测试<br>确保无回归"]
    end
    subgraph STEP4["STEP 4: 覆盖率检查"]
        CHECK["mvn test-coverage<br>未达标补充测试"]
    end

    WRITE_TEST --> RUN_TEST --> WRITE_IMPL --> VERIFY --> OPTIMIZE --> REGRESS --> CHECK

    style STEP1 fill:#ffcdd2,stroke:#c62828
    style STEP2 fill:#c8e6c9,stroke:#2e7d32
